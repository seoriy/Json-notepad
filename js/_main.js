...
(function () {
    ...
    function fieldService() {
        return {
            isSimpleField: function (obj) {
                return angular.isString(obj) || angular.isNumber(obj) || angular.isDate(obj);
            },
            isStringField: function (obj) {
                return angular.isString(obj);
            },
            isNumberField: function (obj) {
                return angular.isNumber(obj);
            },
            isDateField: function (obj) {
                return angular.isDate(obj);
            },
            isArrayField: function (obj) {
                return angular.isArray(obj);
            },
            isObjectField: function (obj) {
                return !angular.isArray(obj) && angular.isObject(obj);
            },
            isLabelDefined: function (f) {
                return f == 0 || f;
            },
            isLink: function (str) {
    ...
    function appendJQuery($compile, $scope) {
        $scope.apply(function () {
            var gen = function (tmlVar) {
                $('div.label-' + tmlVar).hover(
    ...
    function RootObj(
    $scope, $http, $q, $timeout, $compile, $analytics, fieldService, sampleData) {
        $scope.samples = sampleData;
        $scope.fieldService = fieldService;

        function init() {
            $scope.toJson = toJson;
            if (!$scope.targetObj) $scope.targetObj = $scope.sample;
        ...
        $scope.loadSample = function (sampleName) {
            if (sampleName && sampleName != '') {
                if ($scope.sampleName == "<None>") {
                    ...
                    $http.get('data/samples/sample-' + sampleName + '.json', {
                        transformResponse: []
                    }).then(function (resp) {
                        $scope.jsonText = resp.data;
                        $scope.parseObject();
                        $analytics.eventTrack('loadSample', {
                            category: 'samples',
                            label: sampleName
                        });
                    });
        ...
        $scope.submitValue = function (val, target, field, index) {
            if (field) target[field] = val;
            else if (index != -1) target[index] = val;
            return true;
        };
    }
    angular.module("jsonEditor", ['angulartics', 'angulartics.google.analytics']).factory("fieldService", fieldService).factory("sampleData", sampleData).directive("butCollapse", butCollapse).controller("RootObj", RootObj);
})();